...
class BridgeService {
    ...
    publishBridge() {
        ...
        this.bridge.on("listening" /* LISTENING */, (port) => {
            log.info("Homebridge v%s (%s) is running on port %s.", (0, version_1.default)(), bridgeConfig.name, port);
        });
    ...
    async loadPlatformAccessories(plugin, platformInstance, platformType, logger) {
        ...
        return new Promise(resolve => {
            ...
            const loadDelayWarningInterval = setInterval(() => {
                log.warn((0, logger_1.getLogPrefix)(plugin.getPluginIdentifier()), "This plugin is taking long time to load and preventing Homebridge from starting. See https://git.io/JtMGR for more info.");
            }, 20000);
